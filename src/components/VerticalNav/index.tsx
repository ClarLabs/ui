...
function NavItemComponent({
  ...
  const renderContent = () => (
    <>
      ...
      {!collapsed && hasChildren && orientation === 'vertical' && (
        <span className={`${styles.expandIcon} ${isExpanded ? styles.expanded : ''}`}>
          ▶
        </span>
      )}
      {!collapsed && hasChildren && orientation === 'horizontal' && (
  ...
  const content = item.href ? (
    <a
      href={item.href}
      className={`${styles.navItem} ${item.disabled ? styles.disabled : ''} ${level > 0 ? styles.nested : ''}`}
      onClick={handleClick}
    >
      {renderContent()}
    </a>
